...
const EditDeleteTaskList = ({
  ...
  const handleEdit = (e: React.FormEvent<HTMLFormElement>) => {
    ...
    axios
      .put("/api/tasks-list", { id: tasklist.id, ...formData })
      .then(() => {
        setEditDialog(false);
  ...
  const handleDelete = async (e: React.MouseEvent) => {
    ...
    axios
      .delete(`/api/tasks-list/${tasklist.id}`)
      .then(() => {
  ...
  return (
    <div className="flex flex-row items-center">
     ...
     <Link
     href={`/to-dos/${tasklist?.id}`}
     target="_blank"
     >
      <IoMdOpen
      size={18}
      className="text-black hover:text-black/95
      dark:text-white dark:hover:text-white/95"
      />
     </Link>

      <Edit2
        size={taskListIconSize}
      ...
      <Dialog
        open={openEditDialog}
        onOpenChange={() => setEditDialog(!openEditDialog)}
      >
        ...
        <DialogContent>
          ...
          <form className="flex flex-col gap-6 mt-8" onSubmit={handleEdit}>
